        ...
        <b-row>
            ...
            >
                ...
                <b-th rowspan="1">환자 상태</b-th> -->
                        ...
                        <b-tr>
                            <b-td colspan="1">{{
                                surgeryData[0].patient_name
                            }}</b-td>
                            <b-td colspan="1">{{
                                surgeryData[0].patient_age
                            }}</b-td>
                            <b-td colspan="1">{{
                                surgeryData[0].patient_sex
                            }}</b-td>
                            <b-td colspan="1">{{
                                surgeryData[0].patient_status
                            }}</b-td>
                        </b-tr>
                    </b-tbody>
                </b-table-simple>
                ...
                <p>
                    예정 수술 시간 : {{ surgeryData[0].start_time }} ~
                    {{ surgeryData[0].end_time }}
                </p>
            ...
            >
                ...
                <template #modal-footer="{ ok, cancel, hide }">
                    <b-button
                        variant="danger"
                        @click="cancelEndSurgeryModal(surgeryData[0].id)"
                    >
                        취소
                    </b-button>
          ...
          <template #modal-footer="{ ok, cancel, hide }">
            ...
            <b-colxx xl="12" lg="12" class="mb-4">
                ...
                >
                <p
                    v-if="currentSurgery.record"
                    style="float:right; margin-top:8px"
                >
                    녹화 시작 시간 : {{ currentSurgery.record_time }}
                </p>
                <div class="separator mb-5"></div>
                <template
                    v-for="(i, index) in surgeryData.slice(
                ...
                >
                    <b-card class="mb-3" :key="index">
                        <div
                            v-if="i.status === '1'"
                            style="width:20px; height:20px; border-radius: 100px; background:#81B214"
                            class="mb-3"
                        ></div>
                        <div
                            v-if="i.status === '2'"
                            style="width:20px; height:20px; border-radius: 100px; background:gray"
                            class="mb-3"
                        ></div>
                        <div
                            v-if="i.status === '3'"
                            style="width:20px; height:20px; border-radius: 100px; background:#FF2626"
                            class="mb-3"
                        ></div>
                        <b-table-simple>
                            <b-tbody striped>
    ...
    methods: {
        ...
        async startSurgery(id) {
            if (this.surgeryData[0].status === "1") {
                ...
                this.surgeryData = await api.pathReserv({
                    id,
                    status: "1",
                    surgeryId: this.currentSurgery.surgery_id
                });
                this.nextModal = false;
            }
        },
        ...
        async saveSurgeryReserv() {
            if (this.form.doctorName === null) {
                ...
                } else endTime = currentDate + " " + this.form.endTime;

                await api.saveReserv({
                    ...this.form,
                    startTime,
                    endTime,
                    surgeryId
                ...
                this.surgeryData = await api.getReserv({
                    surgeryId: this.currentSurgery.surgery_id
                });
        ...
        async getSurgery() {
            ...
            this.currentSurgery = JSON.parse(
                JSON.stringify(result[this.currentSurgeryImdex])
            );
            this.currentSurgery.live_urls = this.currentSurgery.live_urls.split(
                ","
            );
        ...
        setRecordModal() {
            if (
                this.surgeryData.length === 0 ||
                this.surgeryData[0].status !== "1"
            )
                alert(
                    "수술중인 일정이 없습니다\n예약 설정 후 다시 시도해주세요"
                );
            else this.recordModal = true;
        },
        async recordStart() {
            let now = moment().format("YYYY-MM-DD HH:mm:ss");
            if (
                now < this.surgeryData[0].start_time ||
                now > this.surgeryData[0].end_time
            ) {
                if (
                    confirm(
                        "예약된 수술시간이 되지 않았습니다\n녹화를 시작하시겠습니까?"
                ...
                await api.saveRecord({
                    ...
                    department: "외과",
                    doctor: this.surgeryData[0].doctor_name
                        ? this.surgeryData[0].doctor_name
                        : "정보 없음",
                    patient_name: this.surgeryData[0].patient_name
                        ? this.surgeryData[0].patient_name
                        : "정보 없음",
                    surgery_desc: "갑상선암",
            ...
            if (this.surgeryData.length && this.surgeryData[0].status === "2") {
                    ...
                    if (
                        ...
                        parseInt(
                            localStorage.getItem(
                                this.currentSurgery.surgery_name
                            )
                        )
                    ) {
                        this.nextModal = true;
                ...
                if (timeOutList.length > 0) {
                    if (
                        ...
                        parseInt(
                            localStorage.getItem(
                                this.currentSurgery.surgery_name + "_end"
                            )
                        )
                    ) {
